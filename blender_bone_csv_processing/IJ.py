...
def unreal_process_csv_batch(df):
    ...
    df['mid_hip_Y'] = (df['thigh_out_r_RelLoc_Y'] + df['thigh_out_r_RelLoc_Y']) / 2
...
def main():
    df=pd.read_csv("Getting up.csv")
    df = unreal_process_csv_batch(df)
    df.to_csv("Getting up_normalized.csv", index=True)
